...
export VARPODMANVER=`curl https://api.github.com/repos/containers/podman/releases/latest | grep tag_name | awk '{print $2}' | tr -d '"' | tr -d ','`
export VARCONMONVER=`curl https://api.github.com/repos/containers/conmon/releases/latest | grep tag_name | awk '{print $2}' | tr -d '"' | tr -d ','` 
