...
create_packages "podman-rootless" "git" "https://podman.io" "https://github.com/containers/podman.git" "containers/podman" "unused" "slirp4netns crun" "Podman is container engine, installed rootless" "none" "mkdir /etc/containers/ \n echo \"user.max_user_namespaces=16384\" > /etc/sysctl.d/podman.conf \n echo \"$SUDO_USER:100000:65536\" > /etc/subgid \n echo \"$SUDO_USER:100000:65536\" > /etc/subuid \n echo '{"default": [{"type": "insecureAcceptAnything"}]}' > /etc/containers/policy.json \n echo \"runtime = crun\n[runtimes]\ncrun =  [\n    \"/usr/local/bin/crun\"\n] \n sysctl -p /etc/sysctl.d/podman.conf \"" "none" "none" "none" "none" 
